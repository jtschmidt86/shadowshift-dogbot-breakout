...
function buildLevel(n){clearLevel(); player.position.set(0,2,0); state.checkpoint=new THREE.Vector3(0,2,0); state.levelCoinsStart=state.coins; state.objectiveComplete=false; state.hasCloak=false; state.hasMagnet=false; state.hasBattery=false;
  ...
  if(n===1){ state.objective='Collect 10 coins then reach portal. Unlock: Shadow Strike'; for(let i=0;i<18;i++) coin((i%6-3)*4,1.4,Math.floor(i/6)*6-8); hole(6,-2,1.1); portal(18,0); state.unlocked.strike=true; }
  if(n===2){ state.objective='Defeat 5 Robo-Pups + collect 15 coins, then portal'; for(let i=0;i<22;i++) coin((i%7-3)*4,1.4,Math.floor(i/7)*8-8); hole(0,4,1.2); coin(-4,1.5,1); coin(0,1.7,4); coin(4,1.5,7); for(let i=0;i<5;i++) enemy(-10+i*5,8,'', 'pup'); portal(18,12); }
  if(n===3){ state.objective='Collect 3 Shadow Tokens, defeat 3 drones, avoid glitch traps. Unlock: Shadow Clone'; for(let i=0;i<16;i++) coin(-16+(i%8)*4,1.4,-8+Math.floor(i/8)*10); hole(-2,2,1.25); hole(10,6,1.15); coin(-6,1.5,-1); coin(-2,1.8,2); coin(2,1.5,5); for(let i=0;i<3;i++) {const p=new THREE.Mesh(new THREE.OctahedronGeometry(0.6),new THREE.MeshBasicMaterial({color:0xab5cff}));p.position.set(-12+i*8,1.5,12);world.group.add(p);world.pickups.push({mesh:p,type:'token'});} for(let i=0;i<3;i++) enemy(8+i*5,-12,45); state.unlocked.clone=true; portal(18,-12); }
  if(n===4){ state.objective='Cross sky bridges, collect 5 orbs, reach portal'; for(let i=0;i<10;i++) tile(i*5-20,6+i*4,4,4,0x3a2f66,0x1c3a64), coin(i*5-20,2.2,6+i*4); hole(-5,14,1.35); hole(9,24,1.35); coin(-9,2.4,11); coin(-5,2.7,14); coin(-1,2.4,17); for(let i=0;i<5;i++){const o=new THREE.Mesh(new THREE.SphereGeometry(0.5),new THREE.MeshBasicMaterial({color:0x63f9ff}));o.position.set(-20+i*10,3.5,10+i*6);world.group.add(o);world.pickups.push({mesh:o,type:'orb'});} for(let i=0;i<6;i++) tile(-12+i*4,-14,3.5,3.5,0x334366,0x173455); portal(28,44); }
  ...
  if(n===6){ state.objective='Break barriers, defeat 5 Junk Bots, get Mind Magnet'; state.checkpoint.set(0,2,4); for(let i=0;i<5;i++) enemy(-10+i*5,-8,55); for(let i=0;i<14;i++) coin(-18+(i%7)*6,1.4,18+Math.floor(i/7)*6); hole(-4,10,1.35); hole(10,2,1.35); const m=new THREE.Mesh(new THREE.BoxGeometry(1.4,1.4,1.4),new THREE.MeshBasicMaterial({color:0xb2f3ff}));m.position.set(18,1.8,12);world.group.add(m);world.pickups.push({mesh:m,type:'magnet'}); portal(22,-12); }
  if(n===7){ state.objective='Avoid electric floors, defeat 6 bots, recharge + battery. Unlock: Dash'; state.checkpoint.set(0,2,2); for(let i=0;i<6;i++) enemy(-15+i*6,14,60); for(let i=0;i<20;i++) coin(-20+(i%10)*4,1.4,-12+Math.floor(i/10)*20); hole(-8,8,1.45); hole(8,14,1.45); hole(2,-2,1.25); const b=new THREE.Mesh(new THREE.CylinderGeometry(0.6,0.6,1.5),new THREE.MeshBasicMaterial({color:0xfff26d}));b.position.set(0,1.7,20);world.group.add(b);world.pickups.push({mesh:b,type:'battery'}); pickup(-18,1.5,16,'health',0xff5f7d); pickup(18,1.5,-16,'energy',0x59e8ff); state.unlocked.dash=true; portal(20,20); }
...
function animate(t=0){ requestAnimationFrame(animate); const dt=0.016; if(state.alive){
  ...
  world.hazards.forEach(h=>{if(h.type==='hole' && new THREE.Vector2(player.position.x-h.mesh.position.x, player.position.z-h.mesh.position.z).length()<h.radius*0.9){player.position.y-=0.35; if(player.position.y<0.2){player.position.copy(state.checkpoint); vy=0; damagePlayer(15);}}});
