...
function makeHero() {
  ...
  const handL = new THREE.Mesh(new THREE.BoxGeometry(0.22, 0.22, 0.22), matPurple); handL.position.set(-0.68, 1.24, 0);
  const handR = handL.clone(); handR.position.x = 0.68;
  const legR = legL.clone(); legR.position.x = 0.25;
  const bootL = new THREE.Mesh(new THREE.BoxGeometry(0.38, 0.2, 0.45), matWhite); bootL.position.set(-0.25, 0.42, 0.04);
  const bootR = bootL.clone(); bootR.position.x = 0.25;
  const chest = new THREE.Mesh(new THREE.OctahedronGeometry(0.14), new THREE.MeshBasicMaterial({ color: 0xa56bff })); chest.position.set(0, 1.9, 0.39);
  ...
  g.add(body, head, visor, armL, armR, handL, handR, legL, legR, bootL, bootR, chest, cape);
...
function clearLevel() {
  if (world.boss && world.boss.userData && world.boss.userData.label) world.boss.userData.label.remove();
...
function addCheckpointMarker(x, y, z) {
  const m = new THREE.Mesh(new THREE.CylinderGeometry(0.45, 0.45, 0.2, 10), new THREE.MeshBasicMaterial({ color: 0x72ffd6 }));
  m.position.set(x, y + 0.2, z);
  world.group.add(m);
  world.checkpoints.push(new THREE.Vector3(x, y + 1.5, z));
}
...
function buildLevel(n) {
  ...
  if (n === 1) {
    ...
    addTile(6, 0.4, 4, 3, 0.6, 3); addTile(10, 0.8, 4, 3, 0.6, 3); // easy stairs
    addCheckpointMarker(12, 1.2, 0);
  ...
  if (n === 2) {
    ...
    addTile(6, 1.2, 6, 4, 1, 4); addTile(10, 2.4, 10, 4, 1, 4); addCoin(6,2.2,6); addCoin(10,3.3,10);
    addCheckpointMarker(8, 1.2, 8);
    addPortal(26, 16);
  }
  if (n === 3) {
    ...
    addTile(-2,1.2,-2,4,1,4); addTile(2,2.2,-6,4,1,4); addCoin(-2,2.1,-2); addCoin(2,3.1,-6);
    addCheckpointMarker(0,1.2,-2);
  ...
  if (n === 4) {
    ...
    addTile(-4, 2.2, 18, 4, 0.8, 6); // ramp-like step platform
    addCheckpointMarker(6, 3.2, 24);
  ...
  if (n === 5) {
    ...
    addTile(4,1.3,16,4,1,4); addTile(8,2.6,20,4,1,4); addTile(12,3.9,24,4,1,4);
    addCoin(4,2.2,16); addCoin(8,3.5,20); addCoin(12,4.8,24);
    addCheckpointMarker(4,1.3,16);
  ...
  if (n === 7) {
    ...
    addCheckpointMarker(4, 0, 12);
    state.unlocked.dash = true; addPortal(22, 22);
  }
  if (n === 8) {
    state.objective = 'Defeat DogBot 9000 and reach portal';
    const b = new THREE.Group();
    const body = new THREE.Mesh(new THREE.BoxGeometry(9, 5.5, 12), new THREE.MeshStandardMaterial({ color: 0x8a8a9f, emissive: 0x220000 }));
    const head = new THREE.Mesh(new THREE.BoxGeometry(4.2, 3.2, 4), new THREE.MeshStandardMaterial({ color: 0x9b9bac }));
    head.position.set(0, 1.1, 7.2);
    const snout = new THREE.Mesh(new THREE.BoxGeometry(2.5, 1.4, 2.1), new THREE.MeshStandardMaterial({ color: 0x7a7a8e })); snout.position.set(0, -0.2, 9.5);
    const earL = new THREE.Mesh(new THREE.BoxGeometry(0.9, 1.2, 0.9), new THREE.MeshStandardMaterial({ color: 0x6f6f80 })); earL.position.set(-1.4, 2.5, 7.4);
    const earR = earL.clone(); earR.position.x = 1.4;
    const eyeMat = new THREE.MeshBasicMaterial({ color: 0xff2c2c });
    const eyeL = new THREE.Mesh(new THREE.BoxGeometry(0.35,0.35,0.2), eyeMat); eyeL.position.set(-0.8,1.1,9.25);
    const eyeR = eyeL.clone(); eyeR.position.x = 0.8;
    const jaw = new THREE.Mesh(new THREE.BoxGeometry(2.6, 0.5, 2), new THREE.MeshStandardMaterial({ color: 0xb5b5c5 })); jaw.position.set(0,-1.1,9.35);
    const core = new THREE.Mesh(new THREE.OctahedronGeometry(0.9), new THREE.MeshBasicMaterial({ color: 0xff3b6f })); core.position.set(0,0.2,4.3);
    const legs=[];
    [[-2.9,-2.8,4], [2.9,-2.8,4], [-2.9,-2.8,-3.8], [2.9,-2.8,-3.8]].forEach(([x,y,z])=>{const l=new THREE.Mesh(new THREE.BoxGeometry(1.6,3.1,1.8),new THREE.MeshStandardMaterial({color:0x676778}));l.position.set(x,y,z);legs.push(l);});
    const tail = new THREE.Mesh(new THREE.BoxGeometry(0.9, 0.9, 6), new THREE.MeshBasicMaterial({ color: 0xff6688 }));
    tail.position.set(0, 0.2, -8.8);
    b.add(body, head, snout, earL, earR, eyeL, eyeR, jaw, core, tail, ...legs);
    b.position.set(0, 6, -28);
    const bossLabel = document.createElement('div'); bossLabel.className='floating-damage'; bossLabel.style.color='#ff8aa0'; bossLabel.textContent='DogBot 9000'; document.body.appendChild(bossLabel);
    b.userData = { hp: 300, cooldown: 1.4, phase: 1, tailHp: 90, tail, active: true, core, label: bossLabel };
...
function animate(t = 0) {
  ...
  if (state.alive) {
    ...
    if (!grounded && player.position.y < -8) respawnFromFall();
  ...
  if (world.boss && world.boss.userData.label) {
    const bp = world.boss.position.clone().add(new THREE.Vector3(0, 7, 0)).project(camera);
    world.boss.userData.label.style.left = `${(bp.x * 0.5 + 0.5) * innerWidth}px`;
    world.boss.userData.label.style.top = `${(-bp.y * 0.5 + 0.5) * innerHeight}px`;
  }
